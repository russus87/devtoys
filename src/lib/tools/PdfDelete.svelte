<script lang="ts">
  ...
  async function deletePages() {
    ...
    const dest = await save({
      defaultPath: "senza-pagine.pdf",
      filters: [{ name: "PDF", extensions: ["pdf"] }],
    });
    if (!dest) return;
    busy = true;
    try {
      await invoke("pdf_delete_pages", { path, dest, pages: selectedPages });
...
<div class="tool">
  ...
  <div class="stack">
    <span class="field-label">Pagine da eliminare (es. 1-3,5,8)</span>
  ...
  <div class="tool-controls">
    <button class="btn primary" onclick={deletePages} disabled={!path || selectedPages.length === 0 || busy}>
      {busy ? "Eliminazione…" : "Elimina pagine"}
    </button>
  </div>
